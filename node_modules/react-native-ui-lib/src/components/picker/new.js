...
const Picker = props => {
  ...
  return <PickerContext.Provider value={contextValue}>
    ...
    ref={pickerExpandable} modalProps={modalProps} expandableContent={expandableModalContent} renderCustomOverlay={renderCustomModal ? _renderCustomModal : undefined} onPress={onPress} testID={testID} {...customPickerProps} disabled={props.editable === false}>
        {renderPicker ? // @ts-expect-error TS throws a weird error, might be an issue with TS
      renderPicker(value, label) : <TextFieldMigrator migrate={migrateTextField} customWarning="RNUILib Picker component's internal TextField will soon be replaced with a new implementation, in order to start the migration - please pass to Picker the 'migrateTextField' prop" ref={forwardedRef} // {...textInputProps}
